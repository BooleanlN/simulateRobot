    ...
    for username,password in accounts:
        ...
        while index < len(course_tds):
            ...
            for xx in range(lll):
                ...
                sreach_window = browser.current_window_handle
                grades = browser.find_elements_by_css_selector("td.cell.c2")

                # 寻找最大分数的测验
                fuckDex = 0
                while fuckDex < len(grades):
                    if float(grades[0].get_attribute("innerText"))>80:
                        break
                    fuckDex += 1
                a_btn = browser.find_elements_by_css_selector("td.lastcol")[fuckDex].find_element_by_tag_name("a")
                a_btn.click()
                ...
                try:
                    ...
                    while jndex < len(answers):
                        ...
                        correct_answer = correct_answers[kindex].get_attribute("innerText")[2:].lstrip()
                        jndex+=1
                        kindex+=2
                        ...
                        for article in articles:
                            ...
                            if "完型填空" in content or "完形填空" in content:
                                ...
                                try:
                                    ...
                                    continue
                            elif ( "阅读理解：选择题" in content )or '翻译' in content or "听力理解：请听下面的对话，根据对话内容从A、B、C三个选项中选出一个最佳选项" in content:
                                # 阅读理解补充对话
                                question_type = 3
                                try:
...
def get_from_type4(article):
    ...
    for item in question_items:
        ...
        if len(question_texts) == len(selects):
            break
    # 正误题无选项
